...
- **Deploy is split and path-filtered: Railway runs the backend, Vercel runs the frontend, and each ignores commits that don't touch its files.** `railway.json` (`build.watchPatterns`) rebuilds the backend only on changes under `backend/`, `src/`, `requirements.txt`, or `downloader_worker.py`; `frontend/vercel.json` (`ignoreCommand: git diff --quiet HEAD^ HEAD .`) builds the frontend only when something under `frontend/` changed. So a frontend-only commit deploys Vercel alone and a backend-only commit deploys Railway alone. **The one exception is a schema/contract change: deploy the backend first.** Bumping `RECIPE_SCHEMA_VERSION` in `frontend/src/hooks/useLocalStorage.ts` to match `Recipe.schema_version` in `src/schemas.py` is a frontend-file edit, so the path filters only deploy Vercel — but it's really a contract change, and if the frontend ships a new version before the backend serves it, the mismatch makes `CacheManager.get_cached_recipe` treat every cached recipe as a miss. Deploy backend → then frontend.
